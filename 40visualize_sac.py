...
# Standalone version of the notebook's "Visualize" section (loads sac_actor.pt
# and plays one episode with the deterministic/mean action, saving it as mp4).

import argparse
import sys

import matplotlib
matplotlib.use('Agg')  # no GUI needed; we just save the animation to a file
import matplotlib.pyplot as plt
import matplotlib.animation as animation

# No system ffmpeg on this machine -- point matplotlib's FFMpegWriter at the
# static binary bundled by the imageio-ffmpeg package instead, so .mp4 saving
# works without a separate ffmpeg install.
...
def show_video_of_model(agent, env_name, actor_path='sac_actor.pt', out_path='CarRacing-v2-SAC.mp4',
    ...
    # `max_seconds` of simulated driving we have to raise the env's own step
    # limit, not just slow down how the recorded frames are played back.
    base_env = gym.make(env_name, continuous=True, render_mode='rgb_array')
    render_fps = base_env.metadata['render_fps']
    base_env.close()
    max_episode_steps = int(max_seconds * render_fps)
    ...
    sim_seconds = len(frames) * skip_frames / render_fps
    print(f"Episode finished: {len(frames)} frames ({sim_seconds:.1f}s simulated), total reward = {total_reward:.2f}")
    ...
    # Each recorded frame represents `skip_frames` physics steps, so playing
    # back at render_fps/skip_frames makes 1 video-second == 1 simulated
    # second (real-time playback), rather than an arbitrary speed.
    playback_fps = render_fps / skip_frames
    anim = animation.FuncAnimation(fig, animate, frames=len(frames))
    if out_path.lower().endswith('.gif'):
        anim.save(out_path, writer=animation.PillowWriter(fps=playback_fps))
    else:
        anim.save(out_path, fps=playback_fps)
    plt.close(fig)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Play a trained SAC CarRacing-v2 agent and save it as an mp4.")
    # train_sac_parallel.py now writes checkpoints into last/ and best/ (see its
    # save_checkpoint()) instead of the project root. Pass --actor best/sac_actor.pt
    # for the best-eval checkpoint instead of the most recent one.
    parser.add_argument('--actor', default='best/sac_actor.pt', help="path to the saved actor checkpoint (e.g. last/sac_actor.pt or best/sac_actor.pt)")
